      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={cx(
            ...
            {t.type === 'success' && <CheckCircle className="text-emerald-500 w-5 h-5 shrink-0" />}
            {t.type === 'error' && <ShieldAlert className="text-red-500 w-5 h-5 shrink-0" />}
            ...
            <button onClick={() => setToasts(prev => prev.filter(x => x.id !== t.id))} className="ml-auto text-gray-400 hover:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
        ))}
  ...
  <div className={cx("sharp-card flex flex-col relative", className)}>
    ...
    {(title || action) && (
      <div className="flex justify-between items-center px-4 py-3 border-b border-gray-800">
        <h3 className="font-semibold text-gray-300 tracking-wide text-sm">{title}</h3>
        {action}
      </div>
    )}
...
const ErrorState = ({ error, onRetry }) => (
  <div className="w-full h-full min-h-[200px] flex flex-col items-center justify-center text-center p-6 border border-red-900/50 bg-red-900/10">
    ...
    <h3 className="text-lg font-medium text-red-400 mb-2">Failed to load data</h3>
    <p className="text-gray-400 text-sm mb-4 max-w-md">{error?.message || "Unknown error occurred"}</p>
    {onRetry && (
      <button onClick={onRetry} className="flex items-center gap-2 px-4 py-2 bg-gray-800 hover:bg-gray-700 text-white border border-gray-700 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <Card statusColor={health?.status === 'UP' ? 'var(--color-success)' : 'var(--color-err)'}>
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-gray-400 text-xs font-semibold tracking-wider">NWDAF STATUS</h3>
        ...
        <Card statusColor={nfsUp === nfsTotal && nfsTotal > 0 ? 'var(--color-success)' : 'var(--color-warn)'}>
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-gray-400 text-xs font-semibold tracking-wider">NF HEALTH</h3>
            <Database className="w-5 h-5 text-gray-400" />
          </div>
          <div className="text-2xl font-bold font-mono mb-1">{nfsUp}/{nfsTotal || '-'} NFs</div>
          <div className="text-emerald-400 text-sm font-semibold mb-3">STABLE</div>
        ...
        <Card statusColor="var(--accent-blue)">
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-gray-400 text-xs font-semibold tracking-wider">THROUGHPUT</h3>
            <Activity className="w-5 h-5 text-blue-400" />
          </div>
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center">
              <span className="text-blue-400 font-mono text-sm">DL:</span>
              <span className="text-xl font-bold text-white"><AnimatedNumber value={throughputHistory[throughputHistory.length-1]?.dl || 0} suffix=" kbps" /></span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-emerald-400 font-mono text-sm">UL:</span>
              <span className="text-xl font-bold text-white"><AnimatedNumber value={throughputHistory[throughputHistory.length-1]?.ul || 0} suffix=" kbps" /></span>
            </div>
        ...
        <Card statusColor={isAnomaly ? 'var(--color-err)' : 'var(--color-success)'} 
          ...
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-gray-400 text-xs font-semibold tracking-wider">ANOMALY STATUS</h3>
            {isAnomaly ? <ShieldAlert className="w-5 h-5 text-red-500" /> : <CheckCircle className="w-5 h-5 text-emerald-500" />}
          </div>
          <div className="flex items-center gap-2 mb-4">
            <span className={cx("text-2xl font-bold", isAnomaly ? "text-red-500" : "text-emerald-500")}>
        ...
        <Card statusColor="var(--color-warn)">
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-gray-400 text-xs font-semibold tracking-wider">MOS SCORE</h3>
          ...
          <div className="text-amber-400 text-sm font-semibold mb-2">GOOD</div>
          <div className="text-sm text-gray-500 font-mono">75% conf.</div>
        </Card>

        {/* NET PERFORMANCE */}
        <Card statusColor="var(--accent-blue)">
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-gray-400 text-xs font-semibold tracking-wider">NET PERFORMANCE</h3>
            <Wifi className="w-5 h-5 text-blue-400" />
          </div>
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-3xl font-bold font-mono text-white"><AnimatedNumber value={metrics?.nwdaf_net_score || 0} /></span>
            <span className="text-gray-500 text-sm">/ 100</span>
          </div>
          <div className="text-blue-400 text-sm font-semibold mb-2">FAIR ▲</div>
          <div className="text-sm text-gray-500 font-mono">82% conf.</div>
        </Card>
      </div>
      ...
      <Card title="LIVE THROUGHPUT (30s Window)" className="h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={throughputHistory} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
            ...
            <RechartsTooltip 
              contentStyle={{ backgroundColor: '#111827', borderColor: '#1f2937', fontFamily: 'JetBrains Mono' }}
              itemStyle={{ color: '#fff' }}
            />
...
const AnalyticsPage = ({ analyticsId }) => {
  ...
  const renderVisualization = () => {
    if (loading && !data) return <SkeletonLoader />;
    if (error && !data) return <ErrorState error={error} onRetry={fetchData} />;
    if (!data || !data.analData) return <div className="p-4 text-gray-500 text-center">No data available</div>;
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <RechartsTooltip cursor={{ fill: '#1f2937' }} contentStyle={{ backgroundColor: '#111827', borderColor: '#1f2937' }} />
          ...
          <div className="flex flex-col gap-6">
            {ad.state === 'INSUFFICIENT_DATA' && (
              <div className="p-6 border border-amber-900/50 bg-amber-900/10 flex flex-col items-center justify-center text-amber-500">
                ...
                <div className="w-64 h-2 bg-gray-800 mt-4"><div className="h-full bg-amber-500" style={{ width: `${(ad.samples_collected/ad.samples_required)*100}%` }}/></div>
              </div>
            )}
            ...
            {(ad.state === 'NORMAL' || ad.state === 'ANOMALY_DETECTED') && (
              <>
                <div className={cx("p-4 border flex items-center gap-4", isAnomaly ? "border-red-500 bg-red-900/20 pulse-ring" : "border-emerald-500 bg-emerald-900/10")}>
                  {isAnomaly ? <ShieldAlert className="w-8 h-8 text-red-500" /> : <CheckCircle className="w-8 h-8 text-emerald-500" />}
                  <div>
                    <h3 className={cx("font-bold text-lg", isAnomaly ? "text-red-500" : "text-emerald-500")}>
                      {isAnomaly ? `ANOMALY DETECTED: ${ad.anomaly_type}` : 'SYSTEM NORMAL'}
                    </h3>
                    <p className="text-gray-400 font-mono mt-1">Confidence: {(ad.confidence * 100).toFixed(1)}% | Avg Score: {ad.avg_anomaly_score?.toFixed(3)}</p>
                  </div>
                </div>
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                      ...
                      <XAxis type="number" dataKey="x" name="DL (kbps)" stroke="#6b7280" />
                      <YAxis type="number" dataKey="y" name="UL (kbps)" stroke="#6b7280" />
                      <ZAxis type="number" dataKey="z" range={[50, 400]} name="Score" />
                      <RechartsTooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ backgroundColor: '#111827', borderColor: '#1f2937' }} />
        ...
        return (
          <div className="flex flex-col items-center justify-center py-8">
            <div className="relative w-64 h-32 overflow-hidden mb-6">
              <div className="absolute inset-0 border-[20px] border-gray-800 rounded-t-full border-b-0" />
            ...
            <div className="text-xl text-gray-400 mt-2 tracking-widest">{ad.mos_category || 'UNKNOWN'}</div>
          </div>
        );
      }
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">{analyticsId}</h1>
            {meta?.ref && settings.show3gpp && <span className="text-xs text-gray-500 font-mono bg-gray-800 px-2 py-0.5 rounded">{meta.ref}</span>}
          </div>
        </div>
        <div className="flex items-center gap-3">
          {analyticsId === 'ABNORMAL_BEHAVIOUR' && (
            <button onClick={handleRetrain} className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-sm font-medium border border-gray-700 transition-colors flex items-center gap-2">
              <RotateCcw className="w-4 h-4" /> Retrain Model
            </button>
          )}
          <button onClick={fetchData} className="p-2 bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-700 transition-colors" title="Manual Refresh">
            <RefreshCw className={cx("w-4 h-4", loading && "animate-spin text-blue-400")} />
          </button>
        </div>
      </div>
      ...
      {/* RAW JSON VIEWER */}
      {showRaw && data && (
        <Card title="Raw JSON Response" action={<button onClick={() => setShowRaw(false)}><X className="w-4 h-4 text-gray-500"/></button>}>
          <pre className="text-xs font-mono text-gray-300 overflow-x-auto p-4 bg-[#0a0e1a] border border-gray-800">
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-medium transition-colors">
        ...
        {loading ? <SkeletonLoader /> : subs.length === 0 ? (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-800 font-mono text-sm">
              {subs.map(s => (
                <tr key={s.subscriptionId} className="hover:bg-gray-800/30">
                  <td className="p-4 text-gray-300">{s.subscriptionId?.substring(0,8)}...</td>
                  <td className="p-4 text-blue-400">{s.analyticsId}</td>
                  <td className="p-4 text-gray-400">{s.notifUri}</td>
                  <td className="p-4"><span className="px-2 py-1 bg-emerald-900/30 text-emerald-400 border border-emerald-800 text-xs">ACTIVE</span></td>
                  <td className="p-4 text-gray-400">{s.repPeriod}s</td>
                  <td className="p-4 text-right">
                    <button onClick={() => handleDelete(s.subscriptionId)} className="text-red-400 hover:text-red-300 p-1">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95">
            <h2 className="text-xl font-bold mb-4 border-b border-gray-800 pb-2">Create Subscription</h2>
            <form onSubmit={handleCreate} className="flex flex-col gap-4">
              <div>
                <label className="block text-sm text-gray-400 mb-1">Analytics ID</label>
                <select 
                  className="w-full bg-gray-800 border border-gray-700 p-2 text-white outline-none focus:border-blue-500 font-mono text-sm"
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">Notification URI</label>
                <input 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Rep Period (s)</label>
                  <input 
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Max Reports</label>
                  <input 
                    type="number" min="0"
                    className="w-full bg-gray-800 border border-gray-700 p-2 text-white outline-none focus:border-blue-500 font-mono text-sm"
              ...
              <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-gray-800">
                ...
                <button type="submit" className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-medium">Submit</button>
              </div>
            </form>
          </div>
        </div>
    ...
    <div className="flex flex-col gap-6 max-w-4xl animate-in fade-in duration-300">
      ...
      <Card title="Connection Configuration" statusColor="var(--accent-blue)">
        <div className="flex flex-col gap-6">
          <div>
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-[#0a0e1a] border border-gray-700 p-2 text-white font-mono text-sm outline-none focus:border-blue-500 transition-colors"
              />
              <button 
                onClick={testConnection}
                className="px-4 py-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 transition-colors whitespace-nowrap"
              >
          ...
          {settings.connectionMode === 'gcloud port-forward' && (
            <div className="p-4 bg-[#0a0e1a] border border-gray-800 relative group">
              <p className="text-xs text-gray-500 mb-2">Run this in your local terminal:</p>
              <code className="font-mono text-sm text-emerald-400 break-all">{gcloudCmd}</code>
              <button onClick={() => copyToClipboard(gcloudCmd)} className="absolute top-2 right-2 p-2 bg-gray-800 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {settings.connectionMode === 'SSH Tunnel' && (
            <div className="p-4 bg-[#0a0e1a] border border-gray-800 relative group">
              <p className="text-xs text-gray-500 mb-2">Run this in your local terminal:</p>
              <code className="font-mono text-sm text-emerald-400">{sshCmd}</code>
              <button onClick={() => copyToClipboard(sshCmd)} className="absolute top-2 right-2 p-2 bg-gray-800 opacity-0 group-hover:opacity-100 transition-opacity">
                <Copy className="w-4 h-4" />
              </button>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card title="Refresh Settings">
          <div className="flex flex-col gap-4">
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-gray-300">Auto-refresh Dashboard</span>
              <input 
                type="checkbox" 
            ...
            <div>
              ...
              <select 
                ...
                className="w-full bg-[#0a0e1a] border border-gray-700 p-2 text-white font-mono text-sm outline-none disabled:opacity-50"
              >
        ...
        <Card title="Display Settings">
          <div className="flex flex-col gap-4">
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-gray-300">Show Raw JSON Panel</span>
              <input 
                type="checkbox" 
            ...
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-gray-300">Show 3GPP References</span>
              <input 
                type="checkbox" 
      ...
      <Card title="About" className="mt-4">
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex-1">
            <h3 className="font-bold mb-4 text-white">Open5GS NWDAF Operations</h3>
          ...
          <div className="flex-1 bg-[#0a0e1a] p-4 border border-gray-800">
            <h4 className="text-xs font-semibold text-gray-500 mb-3 uppercase tracking-wider">3GPP Compliance</h4>
            <ul className="text-sm font-mono flex flex-col gap-2">
              <li className="flex justify-between"><span>TS 23.288 v17</span> <span className="text-emerald-500">7/7 IDs ✅</span></li>
              <li className="flex justify-between"><span>TS 29.520 v17</span> <span className="text-emerald-500">Sub/Analyt ✅</span></li>
              <li className="flex justify-between"><span>TS 29.510 v17</span> <span className="text-emerald-500">NRF Reg ✅</span></li>
              <li className="flex justify-between"><span>TS 28.554 v17</span> <span className="text-emerald-500">KPI Coll ✅</span></li>
            </ul>
          </div>
        </div>
      </Card>
...
const AuthProvider = ({ children }) => {
  ...
  const [users, setUsers] = useState(() => {
    ...
    return [{ email: 'tcckaya8@gmail.com', password: '***REMOVED***' }];
  });
...
const AuthPage = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#0a0e1a] p-4 text-white font-sans bg-grid">
      <div className="w-full max-w-md p-8 bg-[#111827] border border-gray-800 shadow-2xl relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-400 mb-1 tracking-wider">EMAIL ADDRESS</label>
            <div className="relative">
              <Mail className="absolute left-3 top-3 w-5 h-5 text-gray-500" />
              <input 
                type="email" required value={email} onChange={e => setEmail(e.target.value)}
                className="w-full bg-[#0a0e1a] border border-gray-700 p-3 pl-10 text-white outline-none focus:border-blue-500 font-mono text-sm transition-colors"
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          ...
          <button onClick={() => setIsLogin(!isLogin)} className="text-blue-400 hover:text-blue-300 underline underline-offset-4">
...
const TopBar = ({ toggleSidebar }) => {
  ...
  return (
    <header className="h-14 border-b border-gray-800 bg-[#111827] flex items-center justify-between px-4 sticky top-0 z-40">
      <div className="flex items-center gap-4">
        <button onClick={toggleSidebar} className="p-1.5 text-gray-400 hover:text-white md:hidden">
      ...
      <div className="flex items-center gap-4 sm:gap-6">
        <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-gray-900 border border-gray-800 text-xs font-mono text-gray-400">
          <span>MCC:999</span>
          <span className="text-gray-700">|</span>
          <span>MNC:70</span>
        </div>
        
        <div className="flex items-center gap-2 text-xs font-mono bg-[#0a0e1a] px-3 py-1.5 border border-gray-800 rounded-full">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={logout} className="p-1.5 text-gray-400 hover:text-red-400 transition-colors" title="Sign Out">
    ...
    <>
      ...
      <div className="flex flex-col h-screen overflow-hidden bg-grid">
            ...
            <div className="flex flex-1 overflow-hidden">
              {/* Sidebar */}
              <aside className={cx(
                "bg-[#111827] border-r border-gray-800 transition-all duration-300 flex flex-col z-30",
                isSidebarOpen ? "w-64" : "w-16 -ml-16 md:ml-0"
              )}>
                <nav className="flex-1 overflow-y-auto custom-scrollbar py-4 flex flex-col gap-1">
                  {navItems.map((item, i) => {
                    if (item.type === 'divider') return <div key={i} className="h-px bg-gray-800 my-2 mx-4" />;
                    ...
                    return (
                      <button
                        ...
                        className={cx(
                          ...
                          active ? "bg-blue-600/10 text-blue-400 border-blue-500" : "text-gray-400 hover:bg-gray-800 hover:text-gray-200 border-transparent",
